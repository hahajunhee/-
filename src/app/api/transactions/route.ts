  ...
  try {
    ...
    for (const item of items) {
      await query(
        ...
        [txn.id, item.product_id, item.product_name, item.category, item.spec, item.unit, item.qty, item.unit_price, item.material_cost, item.other_cost, item.amount, item.vat_apply, item.vat_amount, item.margin, item.net_profit, !!item.invoice_hidden]
      );
    }
