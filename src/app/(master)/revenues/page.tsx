...
import { Customer, Revenue, RevenueCategory, OPERATION_TYPES, OperationType } from '@/types';
...
export default function RevenuesPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [rRes, cuRes, catRes, sRes] = await Promise.all([
      fetch(`/api/revenues?${params}`),
      fetch('/api/customers'),
      fetch('/api/revenue-categories'),
      fetch('/api/settings'),
    ]);
    setRevenues(await rRes.json());
  ...
  const openEdit = (rev: Revenue) => {
    if (rev.source === 'royalty_auto') {
      toast.error('로열티 자동 매출은 비용 페이지에서 수정해주세요');
      return;
    }
    setEditing(rev);
    const cust = customers.find(c => c.id === rev.customer_id);
    setModalBrand(cust?.brand || '');
    setModalOp((cust?.operation_type as OperationType) || '');
    setForm({
      customer_id: rev.customer_id ? String(rev.customer_id) : '',
      category: rev.category,
      settlement_month: rev.settlement_month,
  ...
  const handleSave = async () => {
    if (!form.category || !form.settlement_month) {
      toast.error('카테고리/정산월은 필수입니다');
      return;
    }
    const url = editing ? `/api/revenues/${editing.id}` : '/api/revenues';
  ...
  const handleDelete = async (rev: Revenue) => {
    if (rev.source === 'royalty_auto') {
      toast.error('로열티 자동 매출은 비용 페이지에서 비용을 삭제해야 함께 삭제됩니다');
      return;
    }
    if (!confirm('정말 삭제하시겠습니까?')) return;
    const res = await fetch(`/api/revenues/${rev.id}`, { method: 'DELETE' });
  ...
  const deleteCategory = async (id: number) => {
    if (!confirm('이 매출 구분을 삭제하시겠습니까?')) return;
    const res = await fetch(`/api/revenue-categories/${id}`, { method: 'DELETE' });
    if (res.ok) { toast.success('삭제되었습니다'); fetchAll(); }
  };

  const totalAmount = revenues.reduce((s, c) => s + Number(c.amount), 0);
  ...
  return (
    <>
      <PageHeader
        title="매출"
        description={`총 ${revenues.length}건 / ${formatKRW(totalAmount)}원`}
        action={
          <div className="flex gap-2">
            <button onClick={() => setCatModalOpen(true)} className="btn-secondary">
              <Settings size={16} /> 매출 구분 관리
            </button>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            <button type="button"
              ...
              {(() => {
                if (customerFilter === 'null') return <span className="flex items-center gap-1 text-amber-700"><Crown size={12}/> 본사</span>;
            ...
            {custDropdownOpen && (
              <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-[300px] overflow-y-auto">
                ...
                <button type="button"
                  onClick={() => { setCustomerFilter('null'); setCustDropdownOpen(false); }}
        ...
        <div className="flex flex-wrap gap-3 items-end pt-2 border-t border-gray-100">
          ...
          {(monthFilter || customerFilter || opFilter || brandFilter) && (
            <button onClick={() => { setMonthFilter(''); setCustomerFilter(''); setOpFilter(''); setBrandFilter(''); }}
              className="btn-secondary">필터 해제</button>
          )}
        </div>
      </div>
      ...
      {Object.keys(byCategory).length > 0 && (
        <div className="card mb-4">
          <h3 className="font-semibold mb-3 text-sm text-gray-700">매출 구분별 합계</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {Object.entries(byCategory).map(([cat, amt]) => (
              <div key={cat} className="text-center p-2 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm font-bold text-blue-700">{formatKRW(amt)}원</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="card p-0 overflow-hidden">
        {loading ? (
          ...
          <table className="data-table">
            ...
            <tbody>
              {revenues.map((r) => {
                const isHq = r.customer_id === null;
                const op = (r.operation_type || '가맹점') as OperationType;
                return (
                <tr key={r.id} className={r.source === 'royalty_auto' ? 'bg-amber-50/30' : ''}>
                  ...
                  <td>
                    ...
                    {r.source === 'royalty_auto' && (
                      <span className="ml-1 inline-block px-1.5 py-0.5 text-[10px] rounded bg-amber-200 text-amber-800">자동</span>
                    )}
                  </td>
                  <td className="text-right font-medium text-blue-700">{formatKRW(r.amount)}원</td>
                  <td className="text-gray-500 max-w-[300px] truncate" title={r.notes}>{r.notes || '-'}</td>
                  <td>
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(r)}
                        disabled={r.source === 'royalty_auto'}
                        className={`p-1.5 rounded ${r.source === 'royalty_auto' ? 'opacity-30 cursor-not-allowed' : 'hover:bg-gray-100'}`}>
                        <Pencil size={14} className="text-gray-500" />
                      </button>
                      <button onClick={() => handleDelete(r)}
                        disabled={r.source === 'royalty_auto'}
                        className={`p-1.5 rounded ${r.source === 'royalty_auto' ? 'opacity-30 cursor-not-allowed' : 'hover:bg-red-50'}`}>
              ...
              {revenues.length === 0 && (
                <tr><td colSpan={7} className="text-center py-8 text-gray-400">매출 내역이 없습니다</td></tr>
              )}
            </tbody>
          </table>
        )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}
        title={editing ? '매출 수정' : '매출 입력'} width="max-w-xl">
        <div className="space-y-4">
          ...
          <div className="relative" ref={modalDropdownRef}>
            ...
            <button type="button"
              ...
              className="form-select flex items-center justify-between w-full text-left">
              {(() => {
                if (form.customer_id === '') return <span className="flex items-center gap-1 text-amber-700 font-medium"><Crown size={12}/> 본사 (SOYANG F&C)</span>;
            ...
            {modalCustOpen && (
              <div className="absolute z-30 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-[260px] overflow-y-auto">
                <button type="button"
                  onClick={() => { setForm({ ...form, customer_id: '' }); setModalCustOpen(false); }}
                  className="w-full text-left px-3 py-2 text-sm text-amber-700 hover:bg-amber-50 border-b flex items-center gap-1">
                  <Crown size={12} /> 본사 (SOYANG F&C)
                </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="form-label">매출 구분 *</label>
          ...
          <div>
            ...
            <input type="number" className="form-input" placeholder="예: 5000000"
              value={form.amount}
      ...
      <Modal open={catModalOpen} onClose={() => setCatModalOpen(false)}
        title="매출 구분 관리" width="max-w-md">
        <div className="space-y-4">
          <div className="flex gap-2">
            <input type="text" className="form-input flex-1" placeholder="새 매출 구분 이름"
              value={newCatName}
